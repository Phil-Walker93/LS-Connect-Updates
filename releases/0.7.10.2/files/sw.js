...
const SHELL = [
  ...
  './v0775.js','./v0775-core.js','./v078.js','./v0781.js','./v079.js','./v0791.js','./v0795.js','./v0710.js','./v07101.js','./v07102.js',
  './config.js','./manifest.webmanifest','./version.json','./icon.svg','./icon-192.png','./icon-512.png'
];
self.addEventListener('install',event=>{
  event.waitUntil((async()=>{await caches.open(CACHE).then(cache=>cache.addAll(SHELL));await self.skipWaiting();})());
});
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(key=>key.startsWith('ls-connect-v')&&key!==CACHE).map(key=>caches.delete(key)));await self.clients.claim();})());
});
...
self.addEventListener('message',event=>{
  ...
  if(data.type==='SHOW_NOTIFICATION'){
    event.waitUntil(self.registration.showNotification(data.title||'LS Connect',{body:data.body||'Neue Aktivität',icon:'./icon-192.png',badge:'./icon-192.png',tag:data.tag||'ls-connect',data:data.data||{}}));
  }
});
self.addEventListener('notificationclick',event=>{
  event.notification.close();
  event.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(windows=>{const existing=windows[0];if(existing)return existing.focus();return clients.openWindow('./');}));
});
