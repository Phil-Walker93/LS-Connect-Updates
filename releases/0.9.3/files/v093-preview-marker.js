/* LS Connect v0.9.3 – isolated RC preview proxy marker */
(function v093PreviewMarker(){
  if(window.__LS_CONNECT_V093_PREVIEW_MARKER__) return;
  window.__LS_CONNECT_V093_PREVIEW_MARKER__=true;

  const RC_VERSION='0.9.1';
  const LOADER_VERSION='0.9.3';
  ...
  function mark(){
    ...
    if(badge){
      ...
      badge.title='Isolierte LS Connect Redesign Vorschau v0.9.3 – Production bleibt unverändert';
    }
  }

  mark();
  [0,300,1000,1800,3200].forEach(ms=>setTimeout(mark,ms));
  window.addEventListener('ls-connect:rc-qa',mark);
  console.info('[LS Connect] v0.9.3 isolated RC preview proxy marker active');
})();
