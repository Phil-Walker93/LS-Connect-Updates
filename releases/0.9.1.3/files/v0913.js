...
(async function v0913Boot(){
  ...
  const load=(version,file)=>new Promise((resolve,reject)=>{
    ...
    script.dataset.lsReleaseFile=marker;
    script.src=`/api/script?version=${encodeURIComponent(version)}&file=${encodeURIComponent(file)}&v=0913-layout-r3`;
    script.async=false;
  ...
  await load('0.9.1.3','v0913-layout-r3.js');
  ...
  window.dispatchEvent(new CustomEvent('ls-connect-release-ready',{detail:{version:LS_CONNECT_V0913_VERSION,feature:'lmh-identity-layout-r3'}}));
  console.info('[LS Connect] v0.9.1.3 LMH identity + layout r3 live');
})().catch(error=>{
