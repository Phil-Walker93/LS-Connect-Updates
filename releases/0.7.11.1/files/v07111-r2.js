...
(function v07111R2ReleaseCenterGuard(){
  ...
  async function safeAutoTests(button){
  ...
  async function safePromote(button){
    const data=await center();
  ...
  async function safeRollback(button){
    const data=await center();
  ...
  document.addEventListener('click',async event=>{
    ...
    try{
      if(auto)await safeAutoTests(button);
      else if(promote)await safePromote(button);
      else if(rollback)await safeRollback(button);
    }catch(error){
