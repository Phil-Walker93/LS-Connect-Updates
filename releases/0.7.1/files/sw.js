...
const SHELL = ['./', './index.html', './styles.css', './app.js', './v04.js', './v05.js', './v06.js',
  './v07.js', './v071.js', './config.js', './manifest.webmanifest',
  './version.json', './icon.svg', './icon-192.png', './icon-512.png'];

self.addEventListener('install', event => {
  event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(SHELL)));
  // Do not skipWaiting here. v0.7 uses the local self-updater for application files.
});
...
self.addEventListener('message', event => {
  ...
  if (data.type === 'GET_VERSION') {
    event.source?.postMessage?.({ type: 'LS_CONNECT_VERSION', version: VERSION });
    return;
  }
